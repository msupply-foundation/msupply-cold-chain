...
export class BleService {
  constructor(manager, utils) {
    ...
    _defineProperty(this, "writeCharacteristic", async (macAddress, command) => {
      return this.manager.writeCharacteristicWithoutResponseForDevice(macAddress, BLUETOOTH.UART_SERVICE_UUID, BLUETOOTH.READ_CHARACTERISTIC_UUID, this.utils.base64FromString(command));
    });

    _defineProperty(this, "monitorCharacteristic", (macAddress, callback) => {
      return new Promise((resolve, reject) => {
        this.manager.monitorCharacteristicForDevice(macAddress, BLUETOOTH.UART_SERVICE_UUID, BLUETOOTH.WRITE_CHARACTERISTIC_UUID, (_, result) => {
    ...
    _defineProperty(this, "downloadLogs", async macAddress => {
      ...
      const result = await this.writeAndMonitor(macAddress, BLUE_MAESTRO.COMMANDS.DOWNLOAD, monitorCallback);
      return result;
    });

    _defineProperty(this, "updateLogInterval", async (macAddress, logInterval) => {
      await this.connectAndDiscoverServices(macAddress);
      const result = await this.writeWithSingleResponse(macAddress, `${BLUE_MAESTRO.COMMANDS.UPDATE_LOG_INTERVAL}${logInterval}`, data => !!this.utils.stringFromBase64(data).match(/interval/i));
      return !!result;
    });

    _defineProperty(this, "blink", async macAddress => {
      await this.connectAndDiscoverServices(macAddress);
      const result = await this.writeWithSingleResponse(macAddress, BLUE_MAESTRO.COMMANDS.BLINK, data => {
    ...
    _defineProperty(this, "getInfo", async macAddress => {
      ...
      const result = await this.writeAndMonitor(macAddress, BLUE_MAESTRO.COMMANDS.INFO, monitorResultCallback);
      return result;
    });

    _defineProperty(this, "toggleButton", async macAddress => {
      await this.connectAndDiscoverServices(macAddress);
      const result = await this.writeWithSingleResponse(macAddress, BLUE_MAESTRO.COMMANDS.DISABLE_BUTTON, data => {
        return !!this.utils.stringFromBase64(data).match(/ok/i);
      });
      return result;
    });
